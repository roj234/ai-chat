import {config, MessageRoles, selectedConversation} from "../../states.js";
import {registerDataImportHandler, importConversationData, downloadFile} from "../../data-exchange.js";
import {$computed, $foreach, $state, $store, $update, appendChildren, debugSymbol, unconscious} from "unconscious";
import {SETTING_UI_CONFIG} from "../../setting-ui.js";
import {updateMessageUI} from "../../components/MessageList.jsx";

import {cloneNamed, getTextContent} from "../../utils.js";
...
import {onConversationChanged, registerTools} from "../../skills.js";
import {showToast} from "../../components/Toast.js";
import {Dropdown} from "../../components/Dropdown.jsx";
import {COMMANDS} from "../../commands.js";
import {createTab} from "../../components/SettingDialog.jsx";
import SimpleModal from "../../components/SimpleModal.jsx";
import {_CharacterEditor, createPanel, _LorebookEditor, _PresetEditor} from "./STPresetPanel.jsx";
		...
		onChanged={(type, index) => {
			if (type === 'd') {
				...
				showOverwriteConfirm(selectedItem, typeStr, () => _kvListGetCached(items[index].id).then(value => {
					...
					Object.assign(selectedItem.value, value);
...
charBar[0].append(<button className={"btn ghost"} onClick={() => {
...
SETTING_UI_CONFIG.filter((item) => item._id === "import").forEach((item) => {
...
onConversationChanged((conv, messages) => {
	...
	const {
		...
		preset,
		lorebooks
	} = charInstance.content;
	const readyObj = $state({
		activatedLorebookItems: $state([])
	});

	const promises = [_kvListGetCached(id).then(item => readyObj.character = item)];
	if (preset >= 0) promises.push(_kvListGetCached(preset).then(item => readyObj.preset = item));
	if (lorebooks.length) {
		readyObj.lorebooks = $state(Array(lorebooks.length));
		for (let i = 0; i < lorebooks.length; i++) {
			const j = i;
			promises.push(_kvListGetCached(lorebooks[i]).then(item => readyObj.lorebooks[j] = item));
		}
	}
...
function StoryConfigPanel(self) {
	// 好丑的界面，不过大概能用，话说这也要用$foreach吗？动态列表管理的开销怕不是比这最多一两百项的数组大多了
	const usedLorebooks = new Set(self.content.lorebooks);

	return <details className={"think"}>
		<summary>
		...
		<div>
			...
			<select onChange={({target}) => {
				const id = parseInt(target.selectedOptions[0].value);
				_kvListGetCached(id).then(item => {
					self.content.preset = id;
					self[_FetchFromDB].preset = item;
					// 有什么方案不立即但是记得保存数据库吗？ 比 $update(messages) 好的那种
				})
			}}>
				<option value={-1} selected={usedLorebooks.has(-1)}>不强制</option>
				{$foreach(presetList, ({id, name}) => <option value={id}
															  selected={usedLorebooks.has(id)}>{name}</option>)}
			</select>
			<br/>
			...
			<select style={"width: 80%"} onChange={() => {
				...
				<option value={-1} selected={self.content.lorebooks.indexOf(-1) >= 0}>内置</option>
				{$foreach(lorebookList, ({id, name}) => <option value={id}>{name}</option>)}
			</select>
		</div>
	</details>;
}
...
MessageRoles["st|char"] = {
	name: "角色卡",
	reactive(self) {
		return !self[_FetchFromDB]?.stable;
	},
	/**
	 *
	...
	compose(self, output, callbacks) {
		callbacks.push((input, output) => {
			...
			if (char.autoMessages?.length) {
				for (let {depth, content} of char.autoMessages) {
				...
				for (let book of pages) {
					...
					if (!found) {
						...
						for (let i = input.length-1; i >= min; i--) {
							...
							found = book.regex ? new RegExp(book.triggers[0]).exec(text) : book.triggers.find(trigger => text.includes(trigger));
							if (found) break;
						}
					}
	...
	getChunks(self, chunks, index) {
		...
		for (let i = 0; i < lorebooks.length; i++){
			...
			if (lorebook == null) {
				chunks.push({
					type: "error",
					error: "错误\n引用的世界书 (#"+self.content.lorebooks[i]+") 不存在"
				});
			} else {
...
MessageRoles["st|greeting"] = {
	...
	getChunks(self, chunks) {
		...
		chunks.push({
			key: self,
			type: "text",
			text: applyMacro(greetings[card.content.greeting], createDefaultCtx(char))
		});
		if (greetings.length > 1) {
			chunks.push({
				key: self,
				type: "branch",
				current: card.content.greeting,
