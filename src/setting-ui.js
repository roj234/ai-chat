...
export const SETTING_UI_CONFIG = [
	{
		id: "generateTitle",
		name: "总结对话并生成标题",
		type: "radio",
...
// 手机上删掉对话框宽度
if (isMobile) {
	const index = SETTING_UI_CONFIG.findIndex(({id}) => id === "width");
	SETTING_UI_CONFIG[index] = {
		type: "element",
...
export const BODY_PARAMETERS = SETTING_UI_CONFIG.filter(({id = "", _tab}) => (_tab === "sampling" && !id.endsWith("#")) || id === "max_tokens");
...
SETTING_UI_CONFIG.forEach(({id, _group, choices}) => {
	if (!id) {
...
requestIdleCallback(() => {
	...
	for (let el of SETTING_UI_CONFIG) {
		if (el.id) {
			keys.add(el.id);
