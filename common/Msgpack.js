...
/**
 * @typedef {(string | MsgpackSchema)[]} MsgpackSchema
 */

/**
 * @typedef {Object} MsgpackDecodeOptions
 * @property {boolean} [bigint=false] - 使用bigint或double
 * @property {MsgpackSchema} [schema=null] - 省略数据布局
...
function decodeRawMsg(dataView, offset, options) {
	...
	const decodeMap = schema ? decodeMapSchema : decodeMapNormal;
 ...
 * Msgpack encode
 * @param {any} data
 * @param {MsgpackSchema} [schema=]
 * @returns {Uint8Array}
 */
function encodeMsg(data, schema) {
	...
	function encodeObject(data) {
		...
		if (schema) {
			...
			for (let key in data) {
				let value = data[key];
				if (value !== undefined) {
					if (!currSchema.findIndex) {
						console.log(currSchema);
					}
					const schemaIndex = currSchema.findIndex(item => (Array.isArray(item) ? item[0] : item) === key);
					if (schemaIndex >= 0) {
						encodeNumber(schemaIndex);

						const val = currSchema[schemaIndex];
						schema = Array.isArray(val) ? val[1] : null;
					} else {
						encodeString(key);
						schema = null;
					}
