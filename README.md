...
- 后端上传的文件不会删除
  - 因为我是根据SHA256存的，而且没做Diff，我不知道RefCount所以删不掉
  - 家庭部署的话，也没啥必要自动删除，隔段时间看看好了
  - （当然其实是可以全表扫描一遍的）

## RPG管线 第一部分：真正的“前端角色卡” (MVP)
...
### 目前的缺陷
- 目前只是一个MVP，甚至PoC，只能在开发环境下通过F12推剧情，代码如下
  - api.generateGalAction(api.messages,"听起来很有意思，你们在吵什么？")
  - 和前端其他部分结合需要去除一些耦合，加入一些hook，才能让插件覆盖输入框机制
- 基于`response_format`的严格模式，换句话说，只有正规API支持，RP玩家使用正规API的……好吧，我不是在骂你，但是真的不多
  - 虽然我提供了非严格模式兼容，但 StreamJson 真的没有任何错误恢复机制，我也不想做，如果你想做可以提交PR
