...
class PageCache {
	...
	invalidate(off, len) {
		if (len <= 0) return;
		const lastPage = (off + len - 1) >> 12;
		for (let page = off >> 12; page <= lastPage; page++) {
...
export class TSDB {
	...
	async findByTime(startTime, endTime, options = {}) {
		...
		const { startOwnerId, endOwnerId, firstRow, lastRow } = options;
		...
		const count = this.#rowCount;
		let lo = firstRow ?? await this.#find(0, count, startTime, 0);
		let hi = lastRow ?? await this.#find(lo, count, endTime, 1);
